...
export function findAll(page, size) {
  return request({
    url: `${root}/price/findMonPrice/${page}/${size}`,
    method: 'get'
  })
}
/* 条件查询*/
export function conditionFind(page, size, queryObj) {
  ...
  return request({
    url: `${root}/price/findMonPrice/${page}/${size}?cropId=` + queryObj.cropId + `&startDate=` + queryObj.startDate + `&endDate=` + queryObj.endDate,
    method: 'get'
  })
}
/* 查询所有品种*/
