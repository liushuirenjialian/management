  ...
  <el-row>
    ...
    <el-col :span="16" class="contentBox">
      <el-tabs type="border-card">
        <el-tab-pane label="监测点类型列表">
          <el-form :inline="true" class="demo-form-inline">
            <el-form-item label="监测点类型名称">
              <el-input v-model="monitorName" placeholder="监测点类型名称" @keyup.enter.native="query"></el-input>
            </el-form-item>
            <el-form-item>
              <el-button type="primary" @click="query">查询</el-button>
              <el-button type="primary" @click="addMonitor">添加监测点类型</el-button>
            </el-form-item>
          </el-form>
          <el-table :key='tableKey' :data="list" v-loading.body="listLoading" border fit highlight-current-row style="width: 100%">
            ...
            <el-table-column align="center" width="300" label="监测点名称">
              <template slot-scope="scope"><span class="pointName" @click="handleCheck(scope.row.id)">{{scope.row.orgName}}</span></template>
            </el-table-column>
            <el-table-column align="center" width="100" label="排序">
              <template slot-scope="scope"><span>{{scope.row.orgRank}}</span></template>
            </el-table-column>
            <el-table-column align="center" width="100" label="等级">
              <template slot-scope="scope"><span>{{scope.row.orgType}}</span></template>
            </el-table-column>
    ...
    <el-dialog :title="dialogTittle[dialogType]" :visible.sync="dialogShow">
      <el-form :model="form" status-icon :rules="rules" ref="form" :inline="true" class="demo-form-inline" label-width="150px">
        ...
        <el-form-item label="监测点名称" prop="orgName" style="width: 100%;">
          <el-input type='text' v-model="form.orgName" style="width: 200%;"></el-input>
        </el-form-item>
        <el-form-item label="是否启用">
          <el-radio v-model="form.isUsed" label="0">是</el-radio>
          <el-radio v-model="form.isUsed" label="1">否</el-radio>
        </el-form-item>
        <el-form-item label="排序" prop="orgRank" style="width: 100%;">
          <el-input type='text' v-model="form.orgRank" style="width: 200%;"></el-input>
        </el-form-item>
        <el-form-item label="等级" style="width: 100%;">
          <el-select v-model="form.orgType" disabled placeholder="请选择" style="width: 185%;">
...
<script>
  ...
  import { treeList, tableList, pointList, add, update, remove } from '@/api/monitoring/typeMaintain'
  export default {
    name: 'pointMaintain',
    data() {
      return {
    ...
    methods: {
      ...
      handleEdit(rowId) {
        ...
        pointList(rowId).then(res => {
          ...
          this.form.orgType = res.data.orgType /* 等级*/
          this.form.orgRank = res.data.orgRank /* 排序*/
      ...
      checkPointName(data) {
        console.log(data)
        console.log(data.id + '---' + data.orgName + '---' + data.orgType)
        this.checkedPoint = data.orgName
        this.checkedPointId = data.id
        this.checkedOrgType = data.orgType
      },
      cancelPoint() {
      ...
      determinePoint() {
        ...
        this.form.orgType = this.checkedOrgType + 1
