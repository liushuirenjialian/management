<template>
  <div class="menu-wrapper">
        ...
        <!-- router -->
        <router-link v-if="item.children.length==0&&item.urlType=='router'"  :key="item.children[0].name">
        <el-menu-item :index="item.url+'/'+item.children[0].path" :to="item.url+'/'+item.children[0].path">
          <svg-icon v-if="item.children[0].meta&&item.children[0].meta.icon" :icon-class="item.children[0].meta.icon"></svg-icon>
          <span v-if="item.children[0].meta&&item.children[0].meta.title" slot="title" :rou="item.urlType">{{item.meta.title}}</span>
        </el-menu-item>
      </router-link>
          <!-- http -->
        ...
        <el-submenu v-else :index="item.name||item.path" :key="item.index">
          ...
          <template v-for="child in item.children" v-if="!child.hidden">
            ...
            <!-- router -->
            <router-link v-else-if="child.urlType=='router'" :to="item.url+'/'+child.path" :key="child.name">
